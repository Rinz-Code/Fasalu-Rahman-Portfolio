...
const loginUser = (username, password) => {
  ...
  return tokenRequest.post(`/api/token/both/`, loginBody)
    .then((response)=> {
      window.window.localStorage.setItem(ACCESS_TOKEN, response.data.access);
      window.window.localStorage.setItem(REFRESH_TOKEN, response.data.refresh);
...
const refreshToken = () => {
  const refreshBody = {"refresh": window.window.localStorage.getItem(REFRESH_TOKEN)}
...
const errorInterceptor = (error) => {
  ...
  if (isCorrectRefreshError(status)) {
    return refreshToken().then((data)=> {
      const headerAuthorization = `Bearer ${window.window.localStorage.getItem(ACCESS_TOKEN)}`;
...
const logoutUser = () => {
  window.window.localStorage.removeItem(ACCESS_TOKEN);
  window.window.localStorage.removeItem(REFRESH_TOKEN);
